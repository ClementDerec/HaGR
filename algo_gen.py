...
class net_gen :
    ...
    def __init__(self, mutation_rate : int, crossover_rate : int,pop_size : int, depth : int, data : list[str], K : int, parents_rate : int,
                 number_iteration : int, elitism_rate: int, mutation_node_rate : int) :
    ...
    def crossover(self, root_node1 : Node, root_node2 : Node) :
        ...
        i, j = random.randint(1, max_nodes), random.randint(1, max_nodes)
    ...
    def return_fitness(tree : Node, desired_class : int , landmarks_list : list[(dict[str, int],int)] ) -> float :
    ...
    def elitism (self, list_of_tree : list[Node], landmarks_list : list[ tuple[dict[str,int], float]]) -> list[Node] :
        ...
        """
        if all(len(item) > 1 for item in list_of_tree):
    ...
    def fit_genetic(self, desired_class : int, landmarks_list : list[dict[str,int]]) -> Node :
        ...
        for i in range(self.number_iteration) :  # condition d'arret
            ...
            trees_fitness = [ (  trees[i]  ,  net_gen.return_fitness( trees[i]   ,  desired_class   ,  landmarks_list    )    )    for i in range(len(trees))   ] # calcule la fitness de chaque arbre 
            ...
            for idx, tree in enumerate(parents):
                if random.randint(1,100) <= self.mutation_rate:
                    net_gen.mutate_logic(self, tree)
                    net_gen.mutate_values(self, tree)
        ...
        trees.sort(reverse=True, key = lambda x : net_gen.return_fitness(x,desired_class,landmarks_list))    # sort by fitness
        return trees[0]
    ...
    def hyperparameters_tuning(self, desired_class : int, landmarks_list : list[dict[str,int]], hyperparameters_variation : dict[ str : (float,float,float) ], hyperpamameters_fixed : dict[ str : float]) -> list[(dict[str,int],float)] :
        ...
        for combination in combinations:
            ...
            fitness = net_gen.return_fitness(arbre, desired_class, landmarks_list)
